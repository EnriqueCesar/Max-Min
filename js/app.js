...
function renderSelected(){const body=$('selectedBody');body.innerHTML='';let totalIdeal=0,totalDaily=0;state.selected.forEach((item,i)=>{const c=calc(item);totalIdeal+=c.ideal;totalDaily+=c.diario;body.insertAdjacentHTML('beforeend',`<tr><td class="num">${i+1}</td><td>${esc(item.art)}</td><td>${esc(item.cat)}</td><td>${fmt(c.ideal)}</td><td><b>${fmt(c.min)}</b></td><td><b>${fmt(c.max)}</b></td><td><span class="pill">${esc(c.pres)}</span><br><small>${esc(c.note)}</small></td><td class="no-print"><button class="del" onclick="removeItem(${i})">Quitar</button></td></tr>`)});
$('totalIdeal').textContent=fmt(totalIdeal);$('totalDaily').textContent=fmt(totalDaily);$('totalItems').textContent=state.selected.length;$('formatLabel').textContent=$('modeFilter').value==='pickpack'?'Pick Pack':'Unidad';renderMarkers();}
...
function updateSubtitle(){const weeks=selectedWeeks().join(', ')||'Todas';$('subtitle').textContent=`${$('storeFilter').value} · ${$('campaign').value} · Semanas ${weeks}`;}
